 ...
 * must be pinned by a real test title in `scripts/*.spec.mjs`. A promise
 * without a pinning assertion is not a promise — this plugin owns the user's
 * KANBAN.json, where "cards are archived, never deleted" and "a broken file
 * fails loud" are exactly the claims that must not drift silently.
 *
 * Rules enforced:
 *   1. `docs/guarantees.md` exists and carries rows `| <id> | <guarantee> | <selector> |`;
 *   2. every selector occurs as a test title in `scripts/*.spec.mjs`;
...
for (const row of rows) {
  if (!row.selector) {
    failures.push(`${row.id}: no test selector`)
    continue
  }
